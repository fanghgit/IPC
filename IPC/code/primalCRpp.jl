...
function obtain_g_new(U, V, X, d1, d2, lambda, rows, vals, m, n_pairs_item)
	g1 = (lambda * n_pairs_item') .* V;
...
function compute_Ha_newnew(a, m, U, X, r, d1, d2, lambda, rows, vals, n_pairs_item)
	Mr = n_pairs_item' .* ones(r, d2);
	Ha = (lambda * vec(Mr)) .* a
#	Ha1 = lambda*a
...
function solve_delta(g, m, U, X, r, d1, d2, lambda, rows, vals, infor_list, n_pairs_item)
...
#@time Hp = compute_Ha_new(p, m, U, X, r, d1, d2, lambda, rows, vals)
		Hp = compute_Ha_newnew(p, m, U, X, r, d1, d2, lambda, rows, vals, n_pairs_item)
...
function update_V(U, V, X, r, d1, d2, lambda, rows, vals, stepsize, cols, n_pairs_user, n_pairs_item)
	...
	infor_list = [];
  	g = obtain_g_new(U, V, X, d1, d2, lambda, rows, vals,m, n_pairs_item)
	delta = solve_delta(vec(g), m, U, X, r, d1, d2, lambda, rows, vals, infor_list, n_pairs_item)
	delta = reshape(delta, size(V))
	prev_obj = objective(m, U, V, X, d1, lambda, rows, vals, n_pairs_user, n_pairs_item)
	...
	#for iter=1:20
		V = Vold - s * delta
		m = comp_m(U, V, X, d1, d2, rows, vals, cols);
		new_obj = objective(m, U, V, X, d1, lambda, rows, vals, n_pairs_user, n_pairs_item)
...
function solve_delta_u(g, D, lambda, i, V, r, d2, vals, X, rows, mm, infor_ui, n_pairs_user)
	# use linear conjugate grad descent
	delta = zeros(size(g)[1])
	rr = -g
	p = -rr
...
#	Hp = obtain_Hs_new(i, V, X, r, d2, rows, vals, lambda, D, p);
	Hp = obtain_Hs_newnew(i, V, X, r, d2, rows, vals, lambda, D, p, mm, infor_ui, n_pairs_user);
...
function objective_u_new(i, X, lambda, rows, vals, ui, mm, n_pairs_user)
	res1 = lambda * n_pairs_user[i] / 2 * (vecnorm(ui) ^ 2)
	tmp = nzrange(X, i)
	d2_bar = rows[tmp];
	vals_d2_bar = vals[tmp];
	len = size(d2_bar)[1];
...
function obtain_g_u_newnew(i, ui, V, X, r, d2, rows, vals, lambda, mm, infor_ui, n_pairs_user)

	g1 = lambda * n_pairs_user[i] * ui
	tmp = nzrange(X, i)
	len = size(tmp)[1];
	if len==0
...
function obtain_Hs_newnew(i, V, X, r, d2, rows, vals, lambda, D, s, mm, infor_ui, n_pairs_user)
	tmp = nzrange(X, i)
	d2_bar = rows[tmp];
	vals_d2_bar = vals[tmp];
	len = size(d2_bar)[1];
...
function update_u(i, ui, V, X, r, d2, lambda, rows, vals, stepsize, mm, n_pairs_user, n_pairs_item)

	new_obj = 0

	infor_ui = precompute_ui(V, X, r, d2, lambda, rows, vals, mm, i);
#	g, D,c = obtain_g_u_new(i, ui, V, X, r, d2, rows, vals, lambda, mm);
	g, D,c = obtain_g_u_newnew(i, ui, V, X, r, d2, rows, vals, lambda, mm, infor_ui, n_pairs_user);

	prev_obj = objective_u_new(i, X, lambda, rows, vals, ui, mm, n_pairs_user);
	if (c == 0.0) || (norm(g)<1e-4)
		return ui, prev_obj, mm
	end
	delta = solve_delta_u(g, D, lambda, i, V, r, d2, vals, X, rows, mm, infor_ui, n_pairs_user)
	...
	#for inneriter=1:20
		...
		new_obj = objective_u_new(i, X, lambda, rows, vals, ui_new, mm_new, n_pairs_user);
		# if ( new_obj < prev_obj)
		# 	break;
		# else
		# 	s/=2.0;
...
function update_U(U, V, X, r, d1, d2, lambda, rows, vals, stepsize, m, n_pairs_user, n_pairs_item)
	tmpV = sqrt(n_pairs_item') .* V;
	total_obj_new = lambda/2*(vecnorm(tmpV)^2)
	obj_new = 0
	...
	for i in 1:d1
		...
		mm = nonzeros(m[:,i]);
		for k in 1:1
			ui, obj_new, mm = update_u(i, ui, V, X, r, d2, lambda, rows, vals, stepsize, mm, n_pairs_user, n_pairs_item);
#			if obj == -1.0
#				break
#			end
#			if k == 1
...
#primalCRpp_subsample("data/ml-1m/train_positive.csv", "data/ml-1m/negsamples1.csv", "data/ml-1m/test.csv", 100, 0.01)
#primalCRpp_subsample("data/ml-10M100K/train_positive.csv", "data/ml-10M100K/negsamples1.csv", "data/ml-10M100K/test.csv", 100, 7000)

function primalCRpp_subsample(train_pos, train_neg, test, r, lambda)
	X_pos = readdlm(train_pos, ',');
	xpos = vec( round(Int64, X_pos[:,1]) );
	ypos = vec( round(Int64, X_pos[:,2]) );
	vpos = vec( round(Int64, X_pos[:,3]) );
	X_neg = readdlm(train_neg, ',');
	...
	# depending on the size of X, read n_users and n_items from python output
	n = max(maximum(x), maximum(xx)); msize = max(maximum(y), maximum(yy));
	println("Training dataset ", train_pos, " and test dataset ", test, " are loaded. \n There are ", n, " users and ", msize, " items in the dataset.")
    ...
    for i = 1:d1
		...
		idx0 = idx[tmp_vals .== -1];
		idx1 = idx[tmp_vals .== 1];
		vals0 = tmp_vals[tmp_vals .== -1];
		vals1 = tmp_vals[tmp_vals .== 1];
		l0 = length(vals0);
		l1 = length(vals1);
		npairs += l0*l1;
        n_pairs_user[i] = l0*l1;
		n_pairs_item[idx0] += l1;
		n_pairs_item[idx1] += l0;
    end
	...
	totaltime = 0.00000;
	println("iter time objective_function pairwise_error HR NDCG")
	...
	for iter in 1:20
		tic();
 		...
 		V, m, nowobj  = update_V(U, V, X, r, d1, d2, lambda, rows, vals, stepsize, cols, n_pairs_user, n_pairs_item)

		tt = toq();
		@printf("time spend on the update of V: %f\n", tt)
		totaltime += tt;

		tic();

 		U, nowobj = update_U(U, V, X, r, d1, d2, lambda, rows, vals, stepsize, m, n_pairs_user, n_pairs_item)

		tt = toq();
		@printf("time spend on the update of U: %f\n", tt)
		totaltime += tt;
		...
		#ndcg = compute_NDCG(U, V, Y, r, d1, d2, rows_t, vals_t, cols_t, ndcg_k)
	 	pairwise_error, hr, ndcg = compute_pairwise_error(U, V, Y, r, d1, d2, rows_t, vals_t, cols_t, hr_k, ndcg_k)
		println("[", iter, ", ", totaltime, ", ", nowobj, ", ", pairwise_error, ", ", hr, ", ", ndcg, "],")
